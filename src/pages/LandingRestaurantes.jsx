...
export default function LandingRestaurantes() {
  ...
  const whatsappPromoja = "5586999560330";
  ...
  async function cadastrarLead(event) {
    ...
    try {
      const { error } = await supabase.from("leads_restaurantes").insert([
        {
          nome: nome.trim(),
      ...
      if (error) {
        console.log(error);
        alert("Não foi possível enviar seu interesse. Tente novamente.");
        setCarregando(false);
        return;
      }
      ...
      console.log("Erro ao cadastrar lead:", erro);
      alert("Não foi possível enviar seu interesse. Tente novamente.");
    } finally {
      setCarregando(false);
    }
                  ...
                  <div className="p-5">
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3">
                        <div className="w-14 h-14 rounded-2xl bg-[#1C1C1C] flex items-center justify-center shadow-sm">
                          <Store className="text-[#FF5A1F]" size={28} />
                        </div>

                        <div>
                          <p className="font-black text-lg text-[#1C1C1C]">
                            Monster Burger
                          </p>
                    ...
                    <div className="bg-[#FFF4EF] rounded-3xl p-5 mt-5 border border-[#FFD6C7]">
                      ...
                      <div className="flex items-end justify-between mt-1 gap-3">
                        ...
                        <div className="bg-[#FF5A1F] text-white px-3 py-2 rounded-2xl font-black text-xs shadow-lg">
                          ECONOMIZE R$15
                        </div>
                      </div>
                    </div>
            ...
            {sucesso ? (
              <div className="text-center py-8">
                ...
                <p className="text-zinc-500 mt-2">
                  Enviamos uma mensagem no WhatsApp informado com o próximo
                  passo.
                </p>
              </div>
            ) : (
              <>
                ...
                <form onSubmit={cadastrarLead} className="mt-6 space-y-4">
                  ...
                  <input
                    type="tel"
                    required
                    placeholder="WhatsApp com DDD. Ex: 22999999999"
                    value={whatsapp}
                  ...
                  <button
                    type="submit"
                    disabled={carregando}
                    className="w-full bg-[#FF5A1F] text-white py-4 rounded-2xl font-black disabled:opacity-60"
                  >
                    {carregando ? "Enviando..." : "Enviar interesse"}
                  </button>

                  <button
                    type="button"
                    onClick={() => setFormAberto(false)}
                    className="w-full bg-zinc-100 text-[#1C1C1C] py-4 rounded-2xl font-black"
                  >
                    Fechar
                  </button>
                </form>
